...
# plot output on RANK 0.
    ...
    #Summed LFPs axes
		    samp_freq_add= 16001.0
		    interval_freq_add= 1.0/samp_freq_add
		    time_freq_add= arange(0,1,interval_freq_add)
		    wave= np.sin(2.0*10.0*np.pi*time_freq_add)
		    print(summed_LFP)
		    print(wave)
		    wave=1E-2*wave*1.9
		    print(wave)
		    summed_LFP= 1E3*summed_LFP
                    summed_LFP=[summed_LFP[i]+wave[i] for i in range(len(summed_LFP))]
		    


		    wave= np.sin(2.0*9.0*np.pi*time_freq_add)
		    wave=1E-2*wave*1.5
		    summed_LFP=[summed_LFP[i]+wave[i] for i in range(len(summed_LFP))]
		    wave= np.sin(2.0*11.0*np.pi*time_freq_add)
		    wave=1E-2*wave*1.5
		    summed_LFP=[summed_LFP[i]+wave[i] for i in range(len(summed_LFP))]


		    #np.savetxt('summed_LFP.txt',summed_LFP) #salvar o dado sem reamostrar
